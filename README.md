...
- 🚀 **开机自启**：可在配置文件或托盘菜单开关
- 📌 **系统托盘菜单**：立即锁定 / 开机自启 / 开启密码 / 暂停 / 配置 / 退出
...
想要密码保护？先在配置文件里把 `password` 设成你的密码（**只能用英文字母和数字**，区分大小写），再右键托盘勾选 **「开启密码」**；之后解锁就需要输入密码 + 回车。

> 密码为空或含中文/符号时，程序会拒绝开启密码保护并弹气泡提示——因为锁屏时那些字符根本输不进去，设了会导致永远解不开锁。
...
| `[security]` | `password` | *(空)* | 解锁密码（明文，仅 `require_password = true` 时生效）。**只能用英文字母和数字**，区分大小写 |
...
**Q：杀毒软件报警？**
A：BlackLock 使用了低级键盘钩子（`WH_KEYBOARD_LL`）来收密码，部分杀软会对此关注。代码开源、无任何网络行为，可自行审阅或从源码编译。

> 本项目**不做代码签名**：Authenticode 证书需付费，而免费的自签名证书不被 SmartScreen 信任、起不到降低误报的作用。若安全软件拦截了键盘钩子，锁定会失败并弹气泡提示（不会出现"黑屏却解不开"的情况）。

**Q：密码输错了有提示吗？**
A：有，会**蜂鸣**一声。不用弹窗/气泡是因为锁定时黑屏全屏置顶，气泡要么被盖住、要么破坏纯黑观感。
...
```
lock/
├─ CMakeLists.txt         # CMake 工程 (仅 MSVC, 静态 CRT)
├─ CMakePresets.json      # 构建预设 (msvc / msvc-x86)
├─ CHANGELOG.md           # 更新日志
├─ cmake/                 # 构建辅助脚本
├─ .github/workflows/     # CI (编译+测试+零依赖/架构校验) 与 Release (标签自动发版)
├─ docs/
│  └─ 软件开发设计文档.md    # 完整设计文档 (SDD)
├─ tests/
│  └─ test_config.c       # 配置层单元测试 (ctest)
└─ src/                   # 源码 (main / config / hotkey / lockwin / keyhook / autostart / tray / util)
   ├─ config.default.ini  # 默认配置模板 (内嵌为资源 + 拷到 dist)
   └─ app.rc / app.manifest / app.ico
```
